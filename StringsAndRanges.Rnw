...
<<echo=FALSE,out.width=10,comment=NA>>=
...
bam.sub2 <- readGappedAlignments(file=NA19240.chr21Bam(),use.names=TRUE, param=ScanBamParam(which=gr,what=c("seq","qual")))
...
gr <- GRanges("chr21", IRanges(start = 39751950, end=40032591))
bam.sub2 <- readGappedAlignments(file=NA19240.chr21Bam(),use.names=TRUE, param=ScanBamParam(which=gr,what="seq"))
...
@
\end{columns}
<<echo=FALSE,fig.height=2,message=FALSE>>=
p2 <- autoplot(shift(ir,5),fill=brewer.pal(n=length(ir),"Set1")) + xlim(5,40)
p2
tracks(pl, p2)
@
\end{frame}
...
p2 <- autoplot(shift(ir,7:1),fill=brewer.pal(n=length(ir),"Set1")) + xlim(5,40)
tracks(pl, p2)
...
\end{columns}
<<echo=FALSE,fig.height=2>>=
p2 <- autoplot(resize(ir,3),fill=brewer.pal(n=length(ir),"Set1")) + xlim(5,40)
tracks(pl, p2)
@
\end{frame}
...
\end{columns}
<<echo=FALSE,fig.height=2>>=
p2 <- autoplot(resize(ir,3,fix="end"),fill=brewer.pal(n=length(ir),"Set1")) + xlim(5,40)
tracks(pl, p2)
@
\end{frame}
...
\begin{frame}[fragile]
<<echo=FALSE,fig.height=2>>=
p2 <- autoplot(cvg) + xlim(5,40)
tracks(pl,autoplot(cvg))
@
\end{frame}
...
<<>>=
bam <- readGappedAlignments(mybam,use.name=TRUE)
@
...
<<>>=
system.time(bam.sub <-
readGappedAlignments(file=mybam,
use.names=TRUE, 
